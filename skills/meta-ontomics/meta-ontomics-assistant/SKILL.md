...
**PRIORITY: Mina sent a reference sample (مینا ۱۳۶۵) as the mandatory precision standard. READ `/data/workspace/meta-ontomics/standard-daghigh-nemone-mina.md` BEFORE every person profile. Match or exceed its depth: narrative flowing prose (not bullet-title style), dual-polarity per dimension (strength+shadow of same trait), live grounding in jozve terms (نویز شک، فیزیک ایمان، نگنتروپی، لا ضرر، بها دادن به خود، خاصه، فرکانس صفر), generational-psychological root for every trait, boundary warnings ("اگر بیش از حد شود"), honest method note ("بررسی الگویی است نه خبر قطعی؛ با تأیید تو خاصه دقیقتر میشود"), two-future paths with entry signs, numbered jozve-based solutions, poetic closing, and final categorized summary (7 categories).**
...
**DELIVERY-IN-CHAT RULE (explicit correction 2026-08-11):** the chat delivery message itself must BE the full flowing-narrative interpretation of every section — never a title list ("تیتروار") followed by explanations. From the very first message, interpret each section (باطن، موقعیتهای درونی، ارتباطات، هفتگانهها، روحیات، سه دیدگاه، مالی/موانع/سکوها، سیر گذشته، دو مسیر، ریشههای علمی، راهکارها، طرحوارهها، نقشه ۹۰ روزه، جمعبندی) in complete flowing prose, section by section, as one continuous narrative.
