...
0. **Run the script bare — never prefix env vars with `$(cat tokenfile)`**: the token file already contains the `GITHUB_TOKEN=` prefix, so `GITHUB_TOKEN=$(cat REDACTED/.github_token) script --now` injects `GITHUB_TOKEN=GITHUB_TOKEN=ghp_...` → the script's own extraction is bypassed and auth fails with "Invalid username or token". Correct invocation is just `REDACTED/scripts/hermes-memory-backup.sh --now` (the script resolves the token itself, env → `.env` → `.github_token`). If the script was ALREADY running fine this way, a sudden auth failure is almost certainly a bad env-prefix, not a broken token — verify with `curl -H "Authorization: token $(grep -m1 '^GITHUB_TOKEN=' REDACTED/.github_token | cut -d= -f2-) " https://api.github.com/user` (expect HTTP 200) BEFORE touching the token or script.
